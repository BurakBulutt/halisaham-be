...
public class UserProfileServiceImpl implements UserProfileService {
    ...
    @Override
    public List<UserProfileDto> getAllByUserIdIn(List<String> userIds) {
        return repository.findAllByUserIdIn(userIds).stream()
                .map(userProfile -> {
                    UserDto userDto = userService.getById(userProfile.getUserId());
                    return UserProfileMapper.toDto(userProfile, userDto);
                }).toList();
    }
    ...
    public UserProfileDto getById(String id) {
        return repository.findById(id).map(userProfile -> {
            UserDto userDto = userService.getById(userProfile.getUserId());
            return UserProfileMapper.toDto(userProfile,userDto);
        }).orElseThrow(() -> new BaseException(MessageCodes.ENTITY_NOT_FOUND, UserProfile.class.getSimpleName(), id));
    }

    @Override
    public UserProfileDto getByUserId(String id) {
        UserDto userDto = userService.getById(id);
        return repository.findByUserId(userDto.getId()).map(userProfile -> UserProfileMapper.toDto(userProfile, userDto)).orElseThrow(() -> new BaseException(MessageCodes.ENTITY_NOT_FOUND, UserProfile.class.getSimpleName(), id));
    }
